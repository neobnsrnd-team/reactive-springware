/**
 * @file repository.ts
 * @description 하나은행 거래내역 조회 API 호출·데이터 가공·모델 변환 담당.
 *
 * 서버 응답(snake_case)을 클라이언트 모델(TransactionItem, camelCase)로 변환한다.
 * API 응답 구조가 바뀌어도 이 파일만 수정하면 hook·page는 변경하지 않아도 된다.
 *
 * @example
 * const page = await hanaTransactionHistoryRepository.getTransactionList(filter, 1);
...
export const hanaTransactionHistoryRepository = {
  /**
