/**
 * @file hook.ts
 * @description 하나은행 거래내역 조회 데이터 패칭·상태 관리 훅.
 *
 * 데모 앱에는 @tanstack/react-query가 설치되어 있지 않으므로
 * Figma 디자인 기준 정적 목업 데이터로 대체한다.
 * 실제 서비스 연동 시: useInfiniteQuery + hanaTransactionHistoryRepository 로 교체한다.
 *
...
export const useHanaTransactionHistoryList = () => {
  /** 현재 적용된 조회 조건 */
