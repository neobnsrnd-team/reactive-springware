/**
 * @file page.tsx
 * @description KB국민은행 거래내역 조회 페이지.
 *
 * Figma: "Hana Bank Transaction History" 디자인을 KB국민은행 브랜드로 적용.
 *
 * 레이아웃 구조:
 * PageLayout (상단 헤더: 뒤로가기 / 타이틀 / 햄버거)
 *   ├── AccountSelectorCard  — KB 주거래 통장 + 계좌번호 + 변경 버튼
...
export function KbTransactionHistoryListPage() {
  ...
  return (
    /* data-brand="kb": KB국민은행 브랜드 토큰 적용 (노랑/골드 계열)
    ...
    <div data-brand="kb" data-domain="banking">
      ...
      >
        <Stack gap="md" className="pb-standard">
           ...
           * loading=isLoadingMore 로 중복 클릭을 방지한다. */}
          {!isError && hasMore && (
