...
Figma 디자인 URL과 API 정보를 Claude에 입력하면 React 코드를 자동 생성하는 AI 개발 워크플로우
...
2. 화면 디자인     구축된 에셋만 사용하여 화면 설계 (디자이너)
       ↓
3. AI 개발         Figma URL + API 정보 → Claude → React 코드 생성 (기초 작업 30%)
       ↓
4. 최종 연동       REST API(70%) 연결 및 비즈니스 로직 완성
```
...
```bash
npm install @ReactPageForge/component-library
```

### 2. CLAUDE.md 설정

`docs/CLAUDE.md` 파일을 프로젝트 루트에 복사한다.

```bash
cp docs/CLAUDE.md {프로젝트 루트}/CLAUDE.md
```

Claude Code는 프로젝트 루트의 `CLAUDE.md`를 자동으로 읽어 코드 생성 규칙을 적용한다.
...
- **Component Library 전용** — 모든 UI는 `@ReactPageForge/component-library`만 사용. HTML 태그 직접 사용 금지.
