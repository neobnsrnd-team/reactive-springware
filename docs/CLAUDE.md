...
`component-map.md`에 정의된 컴포넌트만 생성한다.
정의되지 않은 컴포넌트를 임의로 만들면 디자이너의 의도와 다른 UI가 생성된다.

### 4. 관심사를 명확히 분리한다

- **HTTP 호출·데이터 가공·모델 변환·에러 처리**는 `repositories/` 폴더에서만 한다.
- **데이터 패칭 로직**은 `hooks/`에서만 한다.
- **컴포넌트**는 데이터를 표시하는 역할만 한다.

Repository 패턴을 사용하는 이유: API 응답 구조가 바뀌어도 `repositories/`만 수정하면 되고, Hook과 Page는 변경하지 않아도 된다. 유지보수 비용이 크게 줄어든다.
...
- `pages/` — 라우팅 단위. 비즈니스 로직 포함 금지.
- `components/` — UI 표시 전담. 데이터 패칭 로직 포함 금지.
- `hooks/` — 데이터 패칭·상태 로직 전담.
- `repositories/` — HTTP 호출·데이터 가공·모델 변환·에러 처리 전담.
- `types/` — 타입 정의 전담.
...
- Figma 컴포넌트가 `component-map.md`에 없는 경우
...
이 프로젝트의 실사용자는 내부 구조를 전혀 모르는 외부 개발자다.
코드만 봐도 의도를 파악할 수 있도록 주석을 작성한다.

- 새 파일을 만들 때 파일 상단 JSDoc은 필수다. 외부 개발자가 파일의 역할을 한눈에 파악해야 한다.
- 왜 이 값을 선택했는지 이유가 필요한 상수·기본값에는 인라인 주석을 단다.
- 의도를 오해할 수 있는 로직, 타입 단언(`as`), 방어 코드에는 반드시 설명을 추가한다.
