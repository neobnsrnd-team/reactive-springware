...
- Only modify files inside `reactive-springware/`
...
- **HTTP 호출·데이터 가공·모델 변환·에러 처리**는 `reactive-springware/repositories/` 폴더에서만 한다.
- **데이터 패칭 로직**은 `reactive-springware/hooks/`에서만 한다.
- **컴포넌트**는 데이터를 표시하는 역할만 한다.

Repository 패턴을 사용하는 이유: API 응답 구조가 바뀌어도 `reactive-springware/repositories/`만 수정하면 되고, Hook과 Page는 변경하지 않아도 된다. 유지보수 비용이 크게 줄어든다.
...
- `reactive-springware/pages/` — 라우팅 단위. 비즈니스 로직 포함 금지.
- `reactive-springware/components/` — UI 표시 전담. 데이터 패칭 로직 포함 금지.
- `reactive-springware/hooks/` — 데이터 패칭·상태 로직 전담.
- `reactive-springware/repositories/` — HTTP 호출·데이터 가공·모델 변환·에러 처리 전담.
- `reactive-springware/types/` — 타입 정의 전담.

Page마다 하위 폴더를 만들지 않는다. 폴더가 늘어날수록 공통 컴포넌트의 위치가 모호해진다.
